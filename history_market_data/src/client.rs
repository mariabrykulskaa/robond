...
impl MarketDataClient {
    ...
    /// # Пример
    /// ```no_run
    /// # use market_data::client::MarketDataClient;
    ...
    pub async fn get_bond_info(&self, bond_id: i64) -> Result<Option<BondInfo>> {
        let bond = sqlx::query_as::<_, BondInfo>(
            "SELECT * FROM bond_bond WHERE id = $1"
        )
        .bind(bond_id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn get_bond_by_isin(&self, isin: &str) -> Result<Option<BondInfo>> {
        let bond = sqlx::query_as::<_, BondInfo>(
            "SELECT * FROM bond_bond WHERE isin = $1"
        )
        .bind(isin)
        .fetch_optional(&self.pool)
        .await?;
    ...
    pub async fn get_traded_bonds(&self) -> Result<Vec<BondInfo>> {
        let bonds = sqlx::query_as::<_, BondInfo>(
            "SELECT * FROM bond_bond WHERE is_traded = true"
        )
        .fetch_all(&self.pool)
        .await?;
